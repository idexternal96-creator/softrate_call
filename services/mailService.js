...
const notifyAdminOfRequest = (company) => {
  ...
  return sendEmail({
    to: ADMIN_EMAIL,
    subject: isTrial ? 'New Free Trial Request - TraceCall' : 'New Paid User Registered - TraceCall',
    html: `
...
const notifyCompanyOfApproval = (company) => {
  return sendEmail({
    to: company.email,
    subject: 'Welcome to TraceCall! Your trial is approved',
    html: `
      <h2>Congratulations!</h2>
      <p>Hi ${company.name},</p>
      <p>Your free trial for <strong>${company.companyName}</strong> has been approved by our admin.</p>
      <p>You can now log in and start using TraceCall for the next 7 days.</p>
...
const notifyCompanyOfRejection = (email, name) => {
  return sendEmail({
    to: email,
    subject: 'Update on your TraceCall trial request',
    html: `
      <p>Hi ${name},</p>
      <p>Thank you for your interest in TraceCall. Unfortunately, your request for a free trial could not be approved at this time.</p>
